...
foreach ($array['items'] as $item) {
	...
	foreach ($item->fields as $field) {
        ...
        $label = $label [0];
        if ( $field['type'] == "textarea" && array_key_exists('show', $field['attributes']) && $field['attributes']['show'] == 'yes' && strcasecmp($field['attributes']['main-category-item'], $title_cat_name) == 0 && strlen($field['value']) > 0) {
            /*------------------------------*/
			...
			if ($first_name_candid != null) {
				for ($i=0; $i < count( $first_name_candid) ; $i++) { 
					if ($last_name_candid[$i]['subcategory'] == $field['attributes']['subcategory'] || $first_name_candid[$i]['subcategory'] == $field['attributes']['subcategory']) {
						$stodios[$last_name_candid[$i]['cognome']  . 
			         	' ' . $first_name_candid[$i]['nome'] .'$' . $studio_name ][$subcategory][$subcategory_item][] =  array(
																							'label-field' => $label,
